...
> P1 scope. Order lifecycle updates for one or more wallet addresses. The stream name, filter keys, data fields, and operational guarantees are proposed contracts for engineering review.
...
```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": { "subscriptionId": "sub_01", "stream": "userOrderUpdates" }
}
```
