...
> P2 scope. Market-wide non-funding ledger events for monitoring and analytics. The stream name, filter keys, data fields, and operational guarantees are proposed contracts for engineering review.
...
```json
{
  ...
  "params": {
    "stream": "allUserNonFundingLedgerEvents",
...
```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": { "subscriptionId": "sub_01", "stream": "allUserNonFundingLedgerEvents" }
}
```
...
- `eventTypes` selects the consumer's intended scope. Omitted filters request the stream's default market-wide scope, except user and builder streams, which require their identity filter.
