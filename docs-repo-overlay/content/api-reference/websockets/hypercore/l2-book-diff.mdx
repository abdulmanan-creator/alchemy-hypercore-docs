...
The first message for each subscribed market carries the current aggregated book with `isSnapshot: true`. Install it as local state. Every later message is a diff.
