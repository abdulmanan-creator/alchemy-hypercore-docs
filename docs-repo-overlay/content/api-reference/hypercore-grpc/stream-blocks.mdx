...
* Block number is the ordering primitive — there is no separate counter to track
...
**`height`**: Block number. Also the stream ordering value — compare against your last
processed block to detect a gap.
...
Replay from the stream is available for approximately 48 hours. Beyond that window,
use the JSON-RPC block methods to backfill.
...
| **Range** | Live, plus ~48h replay | Bounded historical lookup |
