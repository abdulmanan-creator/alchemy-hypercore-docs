...
Use `/info` with a `type` discriminator for native-compatible reads. The seven
intentional purpose-built routes are listed separately; the other documented reads
use `/info`.
