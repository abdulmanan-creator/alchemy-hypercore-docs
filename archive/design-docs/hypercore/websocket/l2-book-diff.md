...
> P0 scope. Ordered L2 changes that apply to a previously received book state. The stream name, filter keys, data fields, and operational guarantees are proposed contracts for engineering review.
...
```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": { "subscriptionId": "sub_01", "stream": "l2BookDiff" }
}
```
