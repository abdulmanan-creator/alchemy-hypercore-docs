...
> P2 scope. OHLCV candle updates across the selected market universe. The stream name, filter keys, data fields, and operational guarantees are proposed contracts for engineering review.
...
```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": { "subscriptionId": "sub_01", "stream": "allCandles" }
}
```
...
- `intervals` selects the consumer's intended scope. Omitted filters request the stream's default market-wide scope, except user and builder streams, which require their identity filter.
