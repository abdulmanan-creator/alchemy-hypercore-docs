...
> Return spot-account balances for a user. The route choice, fields, pagination, and retention semantics are draft contracts unless supplied by a native-compatible implementation.
...
curl --request POST '<HYPERCORE_HTTP_URL>/info' \
  ...
  --data '{"type":"spotClearinghouseState","user":"0x1111111111111111111111111111111111111111"}'
```


## Proposed response
```json
{ "balances":[{"coin":"USDC","total":"1000.00","hold":"0"}],"openOrders":[] }
```
