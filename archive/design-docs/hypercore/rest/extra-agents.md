...
> **Draft API contract**
> Return proposed agent and API-wallet state associated with a user. The route choice, fields, pagination, and retention semantics are draft contracts unless supplied by a native-compatible implementation.
...
```bash
curl --request POST '<HYPERCORE_HTTP_URL>/v1/hypercore/extra-agents' \
...
## Proposed response
```json
{ "user":"0x1111111111111111111111111111111111111111","agents":[{"address":"0x4444444444444444444444444444444444444444","name":"market-maker","permissions":["trade"]}] }
```
